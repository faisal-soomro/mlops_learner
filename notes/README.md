...
| [git-internals.md](git-internals.md) | `.git/` layout, content-addressed `objects/` store, the four object types (blob/tree/commit/tag), refs as plain-text pointers, what corruption means, why DVC/LFS/MLflow all reach for the same content-addressed pattern |
| [git-undo.md](git-undo.md) | The five "undo" verbs (reset/revert/restore/checkout/amend), three reset modes, working-tree/index/HEAD model, decision tree, recovering with the reflog |
| [dvc.md](dvc.md) | Cross-day DVC patterns: `.dvc/` layout, content-addressed cache, per-command auto-staging quirks, `dvc remote` for S3-compatible backends, `--local` for secrets, git/DVC handoff, why not to hand-edit config |
